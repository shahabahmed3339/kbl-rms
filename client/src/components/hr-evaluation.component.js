      ...
      <div className="form-box">
        ...
        <div className="form-group cols">
                                <table id="rept">
                                        ...
                                        <tbody>
                                                {
                                                this.state.users3.filter(req => req.Designation == this.state.Designation).map((p, index) => {
                                                    return  <tr key={index}>
                                                              <td>{index+1}</td>
                                                              <td>{p.Name}</td>
                                                              <td>{p.Email}</td>
                                                              <td>{p.Contact}</td>
                                                              <td>
                                                                  <Link className="action2" to={"/hrevaluationform/"+this.state.users3.filter(app => app.CV == p.CV).map(app => app.Name)[0]}>See Details</Link>
                                                              </td>
                                                            </tr>
                                                          })
                                                }
