...
export default class TechSchedule extends Component {
  constructor(props) {
    super(props);
        ...
        axios.get('http://localhost:5000/Department/')
          .then(response1 => {
            if (response1.data.length > 0) {
              this.setState({
                depts: response1.data.map(user1 => user1.Name)
              })
            }
          })
          .catch((error) => {
  ...
  onChangeChecked(e) {
    let newArray = [...this.state.Evaluators, e.target.value];
    if (this.state.Evaluators.includes(e.target.value)) {
  ...
  onSubmit(e) {
    ...
    axios.post('http://localhost:5000/TechSchedule/add', schedule)
      ...
      link.textContent = 'Schedule Other Technical Evaluation';
        ...
        <div className="form-group"> 
          ...
          <div className="form-control">
          {
            this.state.depts.map(dept => {
              return <>
              <label style={{fontWeight:"500"}}>{dept}</label><br/>
              {
              this.state.hrs.filter(hr => hr.Department === dept).map(hr => {
                return <>
                  <input type="checkbox"
                  value={hr.Name}
                  onChange={this.onChangeChecked}/>
                  <label style={{marginLeft:"5px"}}>{hr.Name} ({hr.Designation})</label><br/>
                </>
              })
              }</>
            })
