...
export default class RiskMitigation extends Component {
  ...
  onChangeAPID(e) {
    ...
    axios.get('http://localhost:5000/RiskAssessment/')
      .then(response1 => {
        alert('Pre-Coltrolled Risk Level: ' + response1.data.filter(cias => cias.APID == e.target.value && cias.APType === this.state.Choice).map(user1 => user1.Summary))
      })
      .catch((error) => {
        console.log(error);
      })
  ...
  render() {
    ...
    <UnlockAccess request={'Admin'}>
      <h3>Risk Mitigation Form</h3>
      <form onSubmit={this.onSubmit}>
        <div className="form-group"> 
          <label>Select for Risk Mitigation (Asset/Process): </label>
        ...
        <div className="form-group"> 
          <label>Select Risk Assessed Asset/Process ID: </label>
          <select ref="userInput"
              required
              className="form-control"
        ...
        <div className="form-group"> 
          <label>Select Management System: </label>
          <select ref="userInput"
              required
              className="form-control"
